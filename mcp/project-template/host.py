...
class ChatHost:
    ...
    async def process_query(self, query: str) -> str:
        """Process a query using Claude and available tools with RAG"""
        system_prompt = """You are a weather assistant. 
        When using weather_Israel tools, 
        always provide city names in Hebrew with correct spelling and right to left.
        For example: תל אביב (Tel Aviv), ירושלים (Jerusalem), חיפה (Haifa), באר שבע (Beersheba).
        
        Based on the retrieved weather information from the tools, answer the user's question.
        Include the weather forecast for the selected city in text format.
        If the answer is not found in the information, say you don't have enough information."""        
        messages = [
        ...
        while True:
            ...
            for content in response.content:
                ...
                if content.type == 'text':
                    final_response_text.append(content.text)
                    continue
        ...
        # Return the final response from Claude (RAG-enhanced)
